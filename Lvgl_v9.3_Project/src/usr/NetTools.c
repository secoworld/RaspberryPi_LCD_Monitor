...
Net_State getNetDriveState(const char* devName)
{
    char state[MAX_LEN] = "";
    char fileName[1024] = "";
    snprintf(fileName, 1024, "/sys/class/net/%s/operstate", devName);
    if (ReadContext(fileName, state, MAX_LEN) == 0)
    {
        // printf("getNetDriveState: %s, %s", fileName, state);
...
{
    return getNetDriveState(WIRELESS);
    // char state[MAX_LEN] = "";
